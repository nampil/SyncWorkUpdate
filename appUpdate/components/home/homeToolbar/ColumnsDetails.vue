...
const columnsHide = ref([])
const showColumns = ref(false)
const columnMove = ref(null)
const loading = ref(false)



const showHeaders = computed(() => {
  if(authStore?.userPreferences?.headers?.length) {
    return authStore.userPreferences.headers
  }
  return homeStore.defaultHeaders
})

const hideHeaders = computed(() => {
  if (!homeStore.orderProperties || !columnsShow.value) {
    return []
  }
  return homeStore.orderProperties.filter((property) => {
    return !columnsShow.value
...
const validateShow = computed(() => {
  const column = columnsShow.value.map((c) => {
    const { ...columns } = c
    return columns
  })
  return useIsEqual(column, showHeaders.value)
})

const validateHide = computed(() => {
  const column = columnsHide.value.map((c) => {
    const { ...columns } = c
    return columns
  })
  return useIsEqual(column, hideHeaders.value)
