<template>
  <v-card class="elevation-0">
    ...
    <v-card-text class="pt-7 pb-5">
      ...
      <div class="container-download-with-folders">
        <v-btn
          class="mb-1"
          icon
          small
  ...
  methods: {
    ...
    downloadWithWorker() {
      ...
      this.worker.onmessage = (e) => {
        const { blobZip, folderName, amountReportsFailed, errors } = e.data
      ...
      this.worker.onerror = (e) => {
        ...
        this.$mainAlertError(
          `An error occurred when trying to download the reports. Please try again.`,
          { timeout: -1, bottom: true }
        )
        this.$store.commit('oos/routes/set_showLoadingDownload', false)
      }
