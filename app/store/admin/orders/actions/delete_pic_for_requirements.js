...
export async function delete_pic_for_requirements(
  _,
  { picsToDelete, orderId, jobType, taskId, requirementId }
) {
  const promises = []
  ...
  for (let index = 0; index < picsToDelete.length; index++) {
    ...
    const storageRef = ref(
      _storage,
      `admin/config/orders-templates/${orderId}/${jobType}/${taskId}/requirements/${requirementId}/${codeName}`
    )
