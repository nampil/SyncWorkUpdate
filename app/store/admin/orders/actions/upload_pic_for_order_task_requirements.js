...
export async function upload_pic_for_order_task_requirements(
  _,
  { files, orderId, jobType, taskId, requirementId }
) {
  const urlsPromises = []
  ...
  files.forEach((file) => {
    const storageRef = ref(
      storage,
      `admin/config/orders-templates/${orderId}/${jobType}/${taskId}/requirements/${requirementId}/${file.codeName}`
    )
