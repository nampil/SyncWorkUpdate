<template>
  <div class="customer-templates | h-100 pa-3 overflow-y-auto">
    <v-data-table
      :headers="headers"
      :items="customers"
      fixed-header
    ...
    >
      ...
      <template v-slot:top>
        <v-toolbar flat dense dark color="secondary">
          <v-toolbar-title>{{ $t('Customer Templates') }}</v-toolbar-title>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1
        ? this.$t('New Customer Template')
        : this.$t('Edit Customer Template')
    },
    validateForm() {
    ...
    localNeedToSave() {
      ...
      if (this.editedIndex === -1) {
        return !compareItems(this.newCustomer, this.defaultItem)
      }
      const propertiesToCheck = ['title']
      return propertiesToCheck.some((p) => {
        return !compareItems(this.newCustomer[p], this.editedItem[p])
      })
    },
  },
  ...
  methods: {
    ...
    cleanUp() {
      if (!this.$refs.newCustomer) return
      this.$refs.newCustomer.resetValidation()
      this.$nextTick(() => {
        this.newCustomer = JSON.parse(JSON.stringify(this.defaultItem))
      })
    },
    ...
    async initialize() {
      ...
      try {
        ...
        this.customers = _items && _items.length ? [..._items] : []
        this.loading = false
      } catch (error) {
        // eslint-disable-next-line
        console.log(error)
    ...
    editItem(item) {
      ...
      this.newCustomer.title = this.editedItem.title
    ...
    async deleteItemConfirm() {
      this.loading = true
      const type = 'customers'
      const objectDelete = this.customers[this.editedIndex]
